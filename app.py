...
with st.expander("📂 Cargar Datos"):
    ...
    if uploaded_file_csv:
        try:
            ...
            csv_content = io.StringIO(uploaded_file_csv.getvalue().decode('utf-8'))
            sep = ';' if ';' in csv_content.readline() else ','
            csv_content.seek(0)
            df_csv = pd.read_csv(csv_content, sep=sep)
...
if df_csv is not None and gdf is not None:
    try:
            ...
            for std_name, possible_names in col_mapping.items():
                ...
                for name in possible_names:
                    if name.lower() in df_cols:
                        ...
                        break
                if found_col_name and found_col_name != std_name:
        ...
        gdf = standardize_columns(gdf, col_mapping_gdf)
        df_csv = standardize_columns(df_csv, col_mapping_csv)
...
if df is not None and not df.empty:
    ...
    with tab3:
        ...
        if df is None or df.empty:
            st.info("Por favor, carga los archivos .zip y .csv en la sección 'Cargar Datos'.")
            ...
            if not selected_stations_df.empty:
                ...
                # Para el mapa, usamos el 'df' que ya tiene la geometría y los datos de precipitación
                gdf_map = df[df['Nom_Est'].isin(selected_stations_list)].copy()
                
                # Calcular la precipitación media para el tooltip
                if years_to_analyze_present:
                    gdf_map['Precipitación Media (mm)'] = gdf_map[years_to_analyze_present].mean(axis=1).round(2)
                else:
                    gdf_map['Precipitación Media (mm)'] = 'N/A'

                existing_fields = [f for f in tooltip_fields if f in gdf_map.columns]
                existing_aliases = [tooltip_aliases[i] for i, f in enumerate(tooltip_fields) if f in gdf_map.columns]
