...
with st.expander("📂  Cargar Datos"):
    ...
    if uploaded_file_csv:
        ...
        try:
            ...
            # Renombrar columnas con los nombres correctos del usuario
            df = df.rename(columns={'Mpio': 'municipio', 'NOMBRE_VER': 'vereda'})
            st.warning("No se ha cargado un archivo CSV. Se está utilizando el archivo 'mapaCV.csv' por defecto.")
        except FileNotFoundError:
            st.error("No se ha cargado un archivo CSV y no se encontró 'mapaCV.csv' en el directorio. Por favor, sube un archivo para continuar.")
            df = None
    ...
    if uploaded_zip:
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Buscar el archivo .shp dentro del directorio temporal
...
# Verificar que se hayan cargado los datos para continuar
data_df = None
if df is not None and gdf is not None:
    try:
        data_df = df.merge(gdf, on='Nom_Est', how='left')
        st.success("Datos de CSV y Shapefile unidos exitosamente.")
    except KeyError:
        st.error("No se pudo unir el CSV con el shapefile. Asegúrate de que ambos archivos tengan una columna llamada 'Nom_Est'.")
        data_df = df # Si la unión falla, al menos usar el DataFrame del CSV
elif df is not None:
    data_df = df
...
# El resto de la aplicación se ejecuta solo si hay datos disponibles
if data_df is not None and not data_df.empty:
    
    # Normalizar los nombres de las columnas para evitar errores de espacios
    ...
    if years_present:
        ...
        st.sidebar.warning("No se encontraron columnas de años para la precipitación. Asegúrate de que tus datos las contengan.")
    ...
    with tab1:
        ...
        if selected_stations_df.empty:
            ...
            # Tabla 1: Información básica de las estaciones
            ...
            # Tabla 2: Estadísticas de precipitación por estación
            ...
            # Tabla 3: Estadísticas agregadas por celda
